...
function sl_render_attributes(array $attributes, string $url, int $page, int $size, int $total_pages): void
{
    require("../templates/attributes.php");
}
...
$statement = $connection->query("SELECT COUNT(*) FROM attributes");
...
$statement = $connection->prepare("SELECT id, name FROM attributes LIMIT :offset, :limit");
$statement->bindValue(":offset", ($page - 1) * $page_size, PDO::PARAM_INT);
$statement->bindValue(":limit", $page_size, PDO::PARAM_INT);
$statement->execute();

$attributes = sl_template_escape_array_of_arrays($statement->fetchAll(PDO::FETCH_ASSOC));

sl_template_render_header();
sl_template_render_sidebar();
sl_render_attributes($attributes, "/attributes", $page, $page_size, $total_pages);
